...
// Recupero contatti
$contatti = [];
$stmt_utenti = $conn->prepare("SELECT id, nome, cognome, role FROM users WHERE id != ? ORDER BY nome ASC");
$stmt_utenti->bind_param("i", $utente_id);
$stmt_utenti->execute();
...
<body class="min-h-screen bg-main overflow-hidden flex justify-center text-[#F1F6FF] relative">
    ...
    <header class="fixed top-0 left-0 right-0 z-50">
        <div class="bg-navbar glass-panel rounded-[29px] p-4 lg:p-5 flex items-center justify-between flex-wrap gap-4 mx-4 md:mx-6 lg:mx-8 mt-4">
            <div class="flex items-center gap-4 lg:gap-6">
                ...
                <a href="messaggistica.php" class="relative flex items-center justify-center text-white bg-[#36A482]/20 border border-[#36A482]/50 p-2.5 rounded-xl group shadow-[0_0_15px_rgba(54,164,130,0.3)]">
    ...
    <div class="w-full max-w-[1400px] mt-28 px-4 md:px-8 grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-130px)] pb-6">
        
        <div class="lg:col-span-4 ui-panel glass-panel flex flex-col shadow-2xl h-full border border-white/5 rounded-[24px] overflow-hidden">
            <div class="p-5 border-b border-white/10 bg-[#0A2338]/80 backdrop-blur-md z-10 flex flex-col gap-4">
                <h2 class="text-2xl font-black text-white tracking-wide">Messaggi</h2>
                <div class="relative">
                    ...
                    <input type="text" id="search-contacts" onkeyup="filtraContatti()" placeholder="Cerca un collega..." class="w-full bg-[#04101A] border border-white/10 rounded-xl pl-10 pr-4 py-2.5 text-sm text-white focus:outline-none focus:border-[#36A482] transition-colors shadow-inner">
                </div>
            </div>
            
            <div class="flex-grow overflow-y-auto p-3 flex flex-col gap-2" id="contacts-list">
                <?php foreach($contatti as $c): ?>
                    <div data-nome="<?php echo strtolower($c['nome'] . ' ' . $c['cognome']); ?>" 
                         onclick="apriChat(<?php echo $c['id']; ?>, '<?php echo addslashes($c['nome'] . ' ' . $c['cognome']); ?>', this)" 
                         class="contatto-item p-3 rounded-xl border border-transparent bg-transparent hover:bg-white/5 cursor-pointer transition-all flex items-center gap-4">
                        <div class="relative">
                            <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-[#36A482] to-[#1D7F75] flex items-center justify-center font-black text-lg text-white shadow-lg">
                                <?php echo strtoupper(substr($c['nome'], 0, 1) . substr($c['cognome'], 0, 1)); ?>
                            </div>
                            <span class="absolute -bottom-1 -right-1 w-3.5 h-3.5 bg-green-500 border-2 border-[#0A2338] rounded-full"></span>
                        </div>
                        <div class="flex-grow truncate">
                            <div class="font-bold text-white text-[15px] truncate"><?php echo htmlspecialchars($c['nome'] . ' ' . $c['cognome']); ?></div>
                            <div class="text-[11px] text-[#BFD6E8]/70 font-semibold uppercase tracking-widest mt-0.5 truncate"><?php echo htmlspecialchars($c['role']); ?></div>
                        </div>
                    </div>
                <?php endforeach; ?>
        ...
        <div class="lg:col-span-8 ui-panel glass-panel flex flex-col shadow-2xl h-full border border-white/5 rounded-[24px] overflow-hidden bg-[#04101A]/90 relative">
            ...
            <div id="no-chat-selected" class="absolute inset-0 flex flex-col items-center justify-center text-[#BFD6E8]/30 z-10 bg-chat-pattern">
                <div class="w-24 h-24 bg-white/5 rounded-full flex items-center justify-center mb-6 border border-white/10 shadow-inner">
                ...
                <h3 class="text-xl font-bold text-white mb-2">I tuoi messaggi</h3>
                <p class="text-sm">Seleziona un collega dalla lista per iniziare a chattare.</p>
            </div>

            <div id="chat-container" class="hidden flex-col h-full z-20">
                <div class="bg-[#0A2338]/90 backdrop-blur-md px-6 py-4 border-b border-white/10 flex justify-between items-center z-30 shadow-md">
                    <div class="flex items-center gap-4">
                        <div class="w-10 h-10 rounded-2xl bg-gradient-to-br from-[#36A482] to-[#138C8F] flex items-center justify-center font-bold text-white shadow-inner">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
                        </div>
                        <div>
                            <h2 id="chat-title" class="text-lg font-black text-white leading-tight">Nome Cognome</h2>
                            <div class="text-[11px] text-[#36A482] font-bold uppercase tracking-widest flex items-center gap-1.5 mt-0.5">
                ...
                <div id="chat-messages" class="flex-grow p-6 overflow-y-auto flex flex-col gap-2 chat-pattern relative">
                    </div>

                <div class="p-4 bg-[#0A2338]/90 border-t border-white/10 backdrop-blur-md">
                    <form onsubmit="inviaMessaggioChat(event)" class="flex gap-3 relative items-end">
                        <div class="flex-grow relative">
                            <input type="text" id="chat-input" placeholder="Scrivi un messaggio..." autocomplete="off" class="w-full bg-[#04101A] border border-white/10 rounded-[18px] pl-5 pr-12 py-3.5 text-[15px] text-white focus:outline-none focus:border-[#36A482] transition-colors shadow-inner">
                        </div>
                        <button type="submit" class="w-12 h-12 shrink-0 bg-[#36A482] hover:bg-[#2c8569] text-white rounded-[16px] flex items-center justify-center transition-all shadow-lg hover:shadow-[#36A482]/40 hover:-translate-y-0.5">
    ...
    <script>
        ...
        // Funzione per la barra di ricerca dei contatti
        function filtraContatti() {
            ...
            if(trovati === 0) noFoundMsg.classList.remove('hidden');
            else noFoundMsg.classList.add('hidden');
        }
